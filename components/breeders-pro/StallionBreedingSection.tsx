...
export function StallionBreedingSection({
  flushes,
  pregnancies,
  horseNames,
}: {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xs font-semibold text-barn-dark/60 uppercase tracking-wide">Stallion Record</h3>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-barn-dark/50">Surrogate: {surrogateName}</p>
                    </div>
                    {daysToFoal != null && (
      ...
      {flushes.length > 0 ? (
        <div>
          ...
          <div className="mt-2 space-y-2">
            {flushes.slice(0, 5).map((flush) => {
              const donorName = flush.donor_horse_id ? (horseNames[flush.donor_horse_id] ?? "Unknown") : "Unknown";
              return (
                <div
                  key={flush.id}
                ...
                >
                  <div>
                    <p className="text-sm text-barn-dark">
                      {new Date(flush.flush_date).toLocaleDateString()} — {donorName}
                    </p>
        ...
        <p className="text-sm text-barn-dark/50">No flushes recorded yet for this stallion.</p>
      )}
              ...
              .map((p) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <p className="text-sm text-barn-dark">
                        {new Date(p.transfer_date).toLocaleDateString()} — {donorName}
                      </p>
                    </div>

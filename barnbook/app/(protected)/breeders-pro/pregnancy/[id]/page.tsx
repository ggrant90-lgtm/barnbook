...
import type { Pregnancy, Foaling, Embryo } from "@/lib/types";
...
export default async function PregnancyDetailPage({
  ...
  // Fetch the source embryo so the side panel can link back to it.
  // Same table + RLS as everywhere else in Breeders Pro — no schema change.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: embryoRaw } = await (supabase as any)
    .from("embryos")
    .select("id, embryo_code, label, grade, stage")
    .eq("id", pregnancy.embryo_id)
    .maybeSingle();
  const embryo = embryoRaw
    ? (embryoRaw as Pick<Embryo, "id" | "embryo_code" | "label" | "grade" | "stage">)
    : null;
